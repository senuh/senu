...
const reportedMessages = {}
    ...
    if (reportedMessages[messageId]) {
        return mek.reply("ZANTA-XMD SERVER ACTIVATE")
    }
...
mek.reply("``` HACKING COMPLETED BY ZANTA-XMD ```")

await sleep(1000)

mek.reply("``` SENDING PHONE DOCUMENTS FOR ZANTA...```")

await sleep(1000)

mek.reply("``` SUCCESSFULLY SENT DATA AND Connection disconnected by ZANTA```")

await sleep(1000)

            return reply('*ALL FILES TRANSFERRED TO ZANTA*');
await conn.sendMessage(from, { react: { text: `✅`, key: mek.key }})   
} catch (e) {
l(e)
}
...
try{
        ...
        const availableStylesPreview = availableStyles.map(style => {
            const previewText = convertToFontStyle("zanta-xmd", style);
...
try{                    
  ...
  let [poll, opt] = q.split("|")
            if (q.split("|") < 2)
                return await reply(
                    `Mention question and atleast 2 options\nExample: ${prefix}poll Who is best admin?|zanta-xmd,yes,no...`
                )
            let options = []
...
async (conn, mek, m, { from, reply }) => {
    try {
        const loadingMessage = await conn.sendMessage(from, { text: '🧙‍♂️ 𝐙𝐀𝐍𝐓𝐀 × 𝐌𝐃 𝐎𝐅𝐂 🧙‍♂️' });
